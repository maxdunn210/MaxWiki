class ScaffoldingSandbox
  ...
  attr_accessor :form_action, :singular_name, :suffix, :plural_suffix, :model_instance
  ...
  def default_input_block
    Proc.new { |record, column| "<tr><td><label for=\"#{record}_#{column.name}\">#{column.human_name}</label></td>\n" +
    "<td>#{input(record, column.name)}</td></tr>\n" }
  end
...
class MaxScaffoldGenerator < Rails::Generator::NamedBase
  ...
  def initialize(runtime_args, runtime_options = {})
    ...
    #MD put dashes in controller_name in case two or more words
  ...
  protected
    ...
    def banner
      "Usage: #{$0} max_scaffold ModelName [ControllerName] [action, ...]"
    end
    ...
    def suffix
      #MD I don't know where "options" are set, so make this always true
