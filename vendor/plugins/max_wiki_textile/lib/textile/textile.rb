class MaxWikiTextile < String
    ...
    def block_textile_table( text ) 
        text.gsub!( TABLE_RE ) do |matches|
            ...
            each do |row|
                ...
                #MD Made a change so you don't need a space after the .
    ...
    def inline_textile_span( text ) 
        QTAGS.each do |qtag_rc, ht, qtag_re, rtype, escaped_re|
            text.gsub!( qtag_re ) do |m|
                ...
                atts = shelve( atts ) if not atts.to_s.empty?
    ...
    def glyphs_textile( text, level = 0 )
        ...
        #This method is called frequently with text being empty. So test here and return to speed it up.
        return if text.match(/\A[\s\n]*\Z/m)

        if text !~ HASTAG_MATCH
            htmlesc( text, :NoQuotes )
            pgl text
            footnote_ref text
        else
            codepre = 0
