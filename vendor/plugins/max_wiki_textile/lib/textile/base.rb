class MaxWikiTextile < String
    ...
    #MD Made a change so you don't need a space after the . in any
    # block. For instance, these now work:
    # p=.Centered Text
    # h1.Headline
    ...
    # All the tags that may appear in regular HTML, as well as some tags that we insert
    HTMLTAGS = /(!--|!doctype|a|abbr|acronym|address|applet|area|
    b|base|basefont|bgsound|big|blink|blockquote|body|br|button|
    caption|center|cite|code|col|colspan|
    dd|definition|del|dir|div|dd|dl|dt|em|embed|font|form|frame|frameset|
    h\d|head|
    hr|html|i|iframe|ilayer|img|input|ins|isindex|
    kbb|label|layer|legend|li|link|map|marquee|
    menu|meta|multicol|noframes|nolayer|noscript|object|ol|optgroup|option|p|param|pre|q|
    s|script|select|server|small|spacer|span|strike|strong|style|sub|sup|
    table|tbody|td|textarea|tfoot|th|thead|title|tr|tt|u|ul|var|
    chapter|notextile|redpre)/xi
    OFFTAGS = /(code|pre|kbd|notextile|form|input)/i
    OFFTAG_MATCH = /(?:(<\/#{ OFFTAGS }>)|(<#{ OFFTAGS }[^>]*>))(.*?)(?=<\/?#{ OFFTAGS }|\Z)/mi
    OFFTAG_OPEN = /<#{ OFFTAGS }/
    OFFTAG_CLOSE = /<\/#{ OFFTAGS }/
    
    # HASTAG_MATCH is Quick check to see if text has any special HTML tags we need to process
    # ALLTAG_MATCH breaks apart the text into sections inside the <> and outside and to recursively process them
    #TODO MD Take our ERB stuff here since we are hiding it with the <pre> code
    # For ERB, it is tricky because It starts with <% and ends with %> and \won't match %
    # Also for ERB, the opening <% can be on a different line from the %>
    # Another thing to watch out for with ERB is that inside the <% %> there can be an assignment like =>
    # so we can't just look for a closing >
    #Regexes with ERB and HTMLTAGS
    #HASTAG_MATCH = /<\/?#{ HTMLTAGS }[^\n]*?>/m
    #ALLTAG_MATCH = /(<\/?#{ HTMLTAGS }[^\n]*?>)|.*?(?=<\/?#{ HTMLTAGS }[^\n]*?>|$)/m
    HASTAG_MATCH = /<%|%>|(<\/?#{ HTMLTAGS }[^\n]*?>)/m
    ALLTAG_MATCH = /((<\/?#{ HTMLTAGS }[^\n]*?>)|(<%.*?%>))|.*?((?=<\/?#{ HTMLTAGS }[^\n]*?>|$)|(?=<%.*?%>))/m
    ...
    def htmlesc( str, mode )
        # Don't esc & here. They have already been turned into special strings in incoming_entries
        # and if we do it again here, we will escape the & in special codes like &#8217
        #str.gsub!( '&', '&amp;' )
    ...
    # If there is a forward or backslash at the end of the line, then replace it with a line break
    def soft_break( text)
      text.gsub!( /(.*)[\/\\]$/, "\\1<br />" )
    end
    ...
    def blocks( text, deep_code = false )
			...
			# can end the div with "div."
			text.gsub!( /
			   ^div            # look for "div" at the beginning of a line
    		   \((.*?)\)       # capture the class name in brackets ( ) into $1 
    		   \.\s*           # followed by a dot and maybe spaces
			  (.*?)            # capture all the following stuff into $2
			  ((?=div           # up to the next "div" (but don't match this part so it will be caught in the next iteration)
			  \([^\)]+\)\.\s*) # followed by "(class name)."
			  |(div\.))        # or ended by "div."
			  /mx ) do |blk|
				block_class = %{ class=#{$1.inspect}}
				BLOCK_GROUP_SPLITTER + ( ($2.strip.empty? || block_class.empty?) ? $2 : textile_p('div', block_class, nil, "\n\n#{$2.strip}\n\n") )
			end
	...
	def blk( text, deep_code = false )
      return text if text =~ /shelf[0-9]+shelf/
			
	  plain = text !~ /\A[#*> ]/

      # skip blocks that are complex HTML, preformatted or ERB
      #TODO MD Take out ERB
      if (text =~ /^\s*<\/?(\w+).*>/ and not SIMPLE_HTML_TAGS.include? $1) or (text =~ /^<%.+%>/m)
          text
      else
    ...
    def check_refs( text ) 
		...
		# Puts text in storage and returns its placeholder
		#  e.g. shelve("some text") => shelf1shelf
    def shelve( val ) 
        @shelf << val
        " shelf#{ @shelf.length }shelf"
    end
    
		# Retrieves text from storage using its placeholder
		#  e.g. retrieve("shelf1shelf") => "some text"
    def retrieve( text ) 
        @shelf.each_with_index do |r, i|
            text.gsub!( " shelf#{ i + 1 }shelf" ){|m| r }
        end
    end
    ...
    def add_pre_list( val ) 
        @pre_list << val
        "<redpre##{ @pre_list.length - 1 }>"
    end
