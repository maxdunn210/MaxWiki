...
module SortHelper
  ...
  # 
  # If multiple keys, apply the sort order to the first key and make the rest ascending
  # Keys need to be separated with commas
  #
  def sort_clause()
    keys = session[@sort_name][:key].split(/,\s*/)
    keys.each_with_index {|key, index| key << ' ' + (index == 0 ? session[@sort_name][:order] : 'asc')}
    keys.join(', ')
  end
