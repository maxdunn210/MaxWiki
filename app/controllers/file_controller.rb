...
class FileController < InstikiApplicationController
  ...
  def file
    if params['file']
      # form supplied
      new_file = @wiki.wiki_files.create(params['file'])
      if new_file.valid?
        flash[:notice] = "File '#{@file_name}' successfully uploaded"
  ...
  def import
    if params['file']
      @problems = []
      import_file_name = "#{@wiki.name}-import-#{Time.now.strftime('%Y-%m-%d-%H-%M-%S')}.zip"
      file_yard.upload_file(import_file_name, params['file'])
      import_from_archive(file_yard.file_path(import_file_name))
      if @problems.empty?
        flash[:notice] = 'Import successfully finished'
      else
  ...
  def import_from_archive(archive)
    ...
    while (entry = zip.get_next_entry) do
      ...
      begin
        existing_page = @wiki.read_page(page_name)
        if existing_page
          if existing_page.content == page_content
            ...
            @wiki.revise_page(page_name, page_content, :textile, Time.now, @author)
          end
        else
          @wiki.write_page(page_name, page_content, :textile, Time.now, @author)
        end
      rescue => e
        logger.error(e)
