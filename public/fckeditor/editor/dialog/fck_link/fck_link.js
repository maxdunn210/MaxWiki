...
function LoadSelection()
{
	...
	//**** MD Oct-2007 ****
	else if ( sHRef.substr(0,1) == '/' && sHRef.length > 1 )	// It is a wiki link.
	{
		sType = 'wiki' ;
		GetE('txtWiki').value = sHRef.substr(1);
	}
	//****************
...
function Ok()
{
	...
	switch ( GetE('cmbLinkType').value )
	{
      ...
      // Add a forward slash at front, if not there.
      // Also, if they paste a regular URL in the wiki box, accept it
			if ( sUri.substr(0,1) != '/' && ! oRegex.UrlOnChangeProtocol.exec( sUri ) ) 
		  	sUri = '/' + sUri ;
...
{
	OpenFileBrowser( FCKConfig.PageBrowserURL, FCKConfig.LinkBrowserWindowWidth, FCKConfig.LinkBrowserWindowHeight ) ;
}
//********
